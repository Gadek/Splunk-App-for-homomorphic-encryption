...
class IpReport(Report):
    ...
    def _perform_search(self, service):
        ...
        search = 'search index=' + self.source_index
        create_dict = dict()
        create_dict['rf'] = "source_ip"
        create_dict['earliest_time'] = self.earliest_time
        create_dict['latest_time'] = self.latest_time
        results_dict = dict()
        results_dict['count'] = 0
        results_dict['output_mode'] = "json"
        results_dict['field_list'] = "_raw source_ip"
    ...
    def _send_result(self, service, resultLogs):
        fields="_raw,source_ip,count"
        print(fields)
        search = '| makeresults format=csv data=\"'+fields+'\n'
        for result in resultLogs:
            print(result)
            search += result +'\n'
        search += '\" |collect index=result_ips'
    ...
    def _getHEContext(self):
        ...
        PyfhelUtils.saveHE('HE_context_and_keys/counting_and_groupping', HE , True, True)
    ...
    def _prepare_operation(self, HE, search_result):
        ips_to_count = []
        for result in search_result:
            ips_to_count.append(result['source_ip'])
