...
def main():
    ...
    #try:
    #service = client.Service(host=host, port=port, token=token)
        #service = client.connect(host=host, port=port, username=username, password=password)
    #assert isinstance(service, client.Service)
    #except:
    #    splunk.Intersplunk.generateErrorResults("Błąd łączenia z API")


    if len(sys.argv) != 6:
        splunk.Intersplunk.generateErrorResults("Wrong number of arguments.")
        exit()
    if sys.argv[3] >= sys.argv[4]:
        splunk.Intersplunk.generateErrorResults("Time range error")
        exit()
    ...
    if analyse_bool:
        ...
        p2 = subprocess.run(
            ["/usr/bin/python3", "report.py", option,time1,time2],
            capture_output=False
        )
        print(p2)
        #run_job(service, idx, time1, time2)
        results["_raw"]="Dane zostały zapisane. Zobacz index result_hashes"
    else:
        results["_raw"] = "Jeśli chcesz przeanalizować dane, zaznacz checkbox \"analyse\""
